...
| --- | --- | --- |
| Sticky top bar | Sample/status; edit-mode toggle; `Lagre`; JSON load; reset; dirty indicator | Editing stays local until `Lagre` succeeds against the database; JSON download is separately labeled; FINAL is read-only. Unimplemented JSON import/reset controls are omitted. The original instead writes every change to `localStorage` and makes `Lagre` download JSON. |
...
In a DRAFT preview, **Edit mode** reveals correction controls for source-backed TMB/MSI and the supported `sample.tumourType`/`sample.specimenType` fields. A correction keeps the original source value visible and requires a reason. It stays in page memory until an explicit successful **Lagre** in the database-backed workspace. The ordinary JSON download is disabled while source corrections are pending; the save-recovery working-copy download is separate. Read-only/offline views cannot save to the database. Unmapped LocalApp TMB, CNV/amplification, fusion/splicing and patient-context fields remain explicitly unavailable, rather than accepting invented values. See [current workspace behavior](report-review-workspace.md).
